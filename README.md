...
Автотесты для сайта Wikipedia (Selenium + TestNG) и Android‑приложения Wikipedia (Appium + TestNG).
...
## Веб-тесты (Selenium)
- Сценарии: английская главная страница, поиск статьи “Selenium (software)”, проверка блока Featured article, открытие случайной статьи.
...
## Мобильные тесты (Appium, Wikipedia Android)
- Сценарии: поиск “Selenium” с проверкой результатов и описаний, открытие статьи “Software testing” из поиска, очистка запроса и проверка, что результаты исчезают.
- Подготовка окружения:
  ...
  - Запустите Appium Server (по умолчанию `http://127.0.0.1:4723`, при необходимости можно указать `/wd/hub`, код попробует оба пути):
    ...
    ```
    Если порт занят, освободите его (например, `lsof -i :4723` → `kill <pid>`).
...
- `src/test/java/com/uitesting/tests/web` — веб-тесты TestNG.
- `src/test/java/com/uitesting/tests/mobile` — мобильные тесты TestNG.
